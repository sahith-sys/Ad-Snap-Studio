...
function GenFillEditor() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/features/generative-fill", payload);
      setResultImage(res.data.data.result_url || res.data.data.output_url);
    } catch (err) {
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto">
      ...
      {imageBase64 && (
        ...
        >
          <img src={imageBase64} alt="input" className="w-[512px]" />
      ...
      {resultImage && (
        <div className="mt-6">
          ...
          <img src={resultImage} alt="Result" className="w-[512px] rounded shadow" />
        </div>
      )}
    </div>
  );
